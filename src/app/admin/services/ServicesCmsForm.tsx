...
export default function ServicesCmsForm({ initialServices }: ServicesCmsFormProps) {
  ...
  const openEdit = (item: ServiceItem) => {
    ...
    setFeaturesText(featuresToText(item.features));
  ...
  const saveModal = async () => {
    ...
    if (modal?.mode === 'create') {
      const ok = await run('create', 'Service нэмэгдлээ.', async () => {
        const item = await createServiceItemAction(data);
        setServices((p) => [...p, item]);
      });
      if (ok) closeModal();
    } else if (modal?.mode === 'edit') {
      const id = modal.item.id;
      const ok = await run('edit', 'Service шинэчлэгдлээ.', async () => {
        const item = await updateServiceItemAction(id, data);
        setServices((p) => p.map((s) => (s.id === id ? item : s)));
      });
      if (ok) closeModal();
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    const ok = await run('delete', 'Service устгагдлаа.', async () => {
    ...
    <>
      <div className="p-6 border rounded-2xl border-white/5 bg-surface">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div>
            <h3 className="text-xl font-display font-700 text-foreground">Үйлчилгээнүүд</h3>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                {['#', 'Гарчиг', 'Дэд гарчиг', 'Онцлог', 'Зураг', 'Өнгө', 'Үйлдэл'].map((h) => (
                  <th
                    key={h}
            ...
            <tbody className="divide-y divide-white/5">
              {services.map((s, i) => (
                <tr key={s.id} className="hover:bg-surface-2">
                  ...
                  <td className="px-3 py-3">
                    <div className="inline-flex items-center gap-2 px-2 py-1 text-xs border rounded-md border-white/10 bg-surface-2 text-muted">
                      <span
      ...
      {modal && (
        <ModalShell
          title={modal.mode === 'create' ? 'Шинэ Service' : 'Service засах'}
        ...
        >
          <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className="space-y-5">
              ...
              <div className="p-5 border rounded-2xl border-white/5 bg-surface">
                <label className="block mb-2 font-mono text-xs tracking-wider uppercase text-muted-2">
                  Онцлогууд * (шинэ мөр эсвэл таслалттай бичнэ)
                </label>
                <textarea
                  value={featuresText}
