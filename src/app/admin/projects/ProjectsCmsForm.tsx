...
import { createProjectItemAction, deleteProjectItemAction, updateProjectItemAction, uploadProjectImagesAction } from '@/lib/actions/projects-section';
...
function tagsToText(t: string[]) { return t.join('\n'); }
function parseTags(s: string) { return s.split(/\n|,/).map((t) => t.trim()).filter(Boolean); }

function emptyDraft(): Omit<ProjectItem, 'id'> {
  return { title: '', category: '', year: new Date().getFullYear().toString(), area: '', image: '', imageAlt: '', tags: [] };
}
...
export default function ProjectsCmsForm({ initialProjects }: ProjectsCmsFormProps) {
  ...
  const closeModal = () => {
    setModal(null); setDraft(emptyDraft()); setTagsText('');
    setUploadState('idle'); setUploadError(null); setFormError(null); setSelectedFileName('');
    if (fileRef.current) fileRef.current.value = '';
  };

  const openCreate = () => { setDraft(emptyDraft()); setTagsText(''); setModal({ mode: 'create' }); setUploadState('idle'); setUploadError(null); setFormError(null); setSelectedFileName(''); };
  const openEdit = (item: ProjectItem) => { setDraft({ title: item.title, category: item.category, year: item.year, area: item.area, image: item.image, imageAlt: item.imageAlt, tags: item.tags }); setTagsText(tagsToText(item.tags)); setModal({ mode: 'edit', item }); setUploadState('idle'); setUploadError(null); setFormError(null); setSelectedFileName(''); };
  ...
  const saveModal = async () => {
    const data: Omit<ProjectItem, 'id'> = { title: draft.title.trim(), category: draft.category.trim(), year: draft.year.trim(), area: draft.area.trim(), image: draft.image.trim(), imageAlt: draft.imageAlt.trim(), tags: parseTags(tagsText) };
    if (!data.title || !data.category || !data.year || !data.image) { setFormError('Гарчиг, ангилал, он, зургийг заавал бөглөнө үү.'); return; }
    if (modal?.mode === 'create') {
      const ok = await run('create', 'Төсөл нэмэгдлээ.', async () => { const item = await createProjectItemAction(data); setProjects((p) => [...p, item]); });
      if (ok) closeModal();
    } else if (modal?.mode === 'edit') {
      const id = modal.item.id;
      const ok = await run('edit', 'Төсөл шинэчлэгдлээ.', async () => { const item = await updateProjectItemAction(id, data); setProjects((p) => p.map((pr) => pr.id === id ? item : pr)); });
      if (ok) closeModal();
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    const ok = await run('delete', 'Төсөл устгагдлаа.', async () => { await deleteProjectItemAction(id); setProjects((p) => p.filter((pr) => pr.id !== id)); });
    ...
    <>
      <div className="rounded-2xl border border-white/5 bg-surface p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="font-display text-xl font-700 text-foreground">Төслийн жагсаалт</h3>
            <p className="mt-1 text-sm text-muted">Гарчиг, ангилал, он, талбай, зураг, таг утгуудыг удирдана.</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="tag-badge">{projects.length} мөр</span>
            <button type="button" onClick={openCreate} className="btn-primary" disabled={isMutating}>Шинэ төсөл нэмэх</button>
          </div>
        </div>
              ...
              {projects.map((p, i) => (
                <tr key={p.id} className="hover:bg-surface-2">
                  ...
                  <td className="px-3 py-3">
                    {p.image && <div className="relative h-12 w-20 overflow-hidden rounded-lg border border-white/5"><AppImage src={p.image} alt={p.imageAlt} fill className="object-cover" /></div>}
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex gap-2">
                      <button type="button" onClick={() => openEdit(p)} className="rounded-md border border-blue-400/30 bg-blue-400/10 px-2 py-1 text-xs text-blue-200 hover:bg-blue-400/20" disabled={isMutating}>Засах</button>
                      <button type="button" onClick={() => setDeleteTarget(p)} className="rounded-md border border-red-400/30 bg-red-400/10 px-2 py-1 text-xs text-red-200 hover:bg-red-400/20" disabled={isMutating}>Устгах</button>
                    </div>
                  </td>
                </tr>
              ))}
              {projects.length === 0 && <tr><td colSpan={7} className="px-3 py-10 text-center text-sm text-muted">Төсөл байхгүй байна.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {modal && (
        <ModalShell title={modal.mode === 'create' ? 'Шинэ төсөл' : 'Төсөл засах'} subtitle="Гарчиг, ангилал, он, зургийг бөглөнө үү." onClose={closeModal}>
          <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className="space-y-5">
              <div className="rounded-2xl border border-white/5 bg-surface p-5">
                <label className="mb-2 block text-xs font-mono uppercase tracking-wider text-muted-2">Гарчиг *</label>
                <input type="text" value={draft.title} onChange={(e) => setDraft((p) => ({ ...p, title: e.target.value }))} className="input-field" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl border border-white/5 bg-surface p-5">
                  <label className="mb-2 block text-xs font-mono uppercase tracking-wider text-muted-2">Ангилал *</label>
                  <input type="text" value={draft.category} onChange={(e) => setDraft((p) => ({ ...p, category: e.target.value }))} className="input-field" />
                </div>
                <div className="rounded-2xl border border-white/5 bg-surface p-5">
                  <label className="mb-2 block text-xs font-mono uppercase tracking-wider text-muted-2">Он *</label>
                  <input type="text" value={draft.year} onChange={(e) => setDraft((p) => ({ ...p, year: e.target.value }))} className="input-field" placeholder="2024" />
                </div>
              </div>
              <div className="rounded-2xl border border-white/5 bg-surface p-5">
                <label className="mb-2 block text-xs font-mono uppercase tracking-wider text-muted-2">Талбай</label>
                <input type="text" value={draft.area} onChange={(e) => setDraft((p) => ({ ...p, area: e.target.value }))} className="input-field" placeholder="12,400 м²" />
              </div>
              <div className="rounded-2xl border border-white/5 bg-surface p-5">
                <label className="mb-2 block text-xs font-mono uppercase tracking-wider text-muted-2">Tags (мөр эсвэл comma)</label>
                <textarea value={tagsText} onChange={(e) => setTagsText(e.target.value)} rows={3} className="input-field resize-none" />
              </div>
              <div className="rounded-2xl border border-white/5 bg-surface p-5">
      ...
      {deleteTarget && (
        <ModalShell title="Төсөл устгах" subtitle="Энэ үйлдэл буцаах боломжгүй." onClose={() => setDeleteTarget(null)}>
          <div className="rounded-2xl border border-white/5 bg-surface p-5">
            <p className="text-sm text-muted">Та <span className="font-semibold text-foreground">&ldquo;{deleteTarget.title}&rdquo;</span> төслийг устгахдаа итгэлтэй байна уу?</p>
          </div>
          <div className="mt-6 flex justify-end gap-3">
